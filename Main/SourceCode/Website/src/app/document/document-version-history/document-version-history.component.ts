...
export class DocumentVersionHistoryComponent extends BaseComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: DocumentInfo,
  ...
  restoreDocumentVersion(version: DocumentVersion) {
    this.sub$.sink = this.commandDialogService.deleteConformationDialog("Are you sure you want to restore this to current version")
      .subscribe((isTrue) => {
        if (isTrue) {
          this.sub$.sink = this.documentService.restoreDocumentVersion(this.data.id, version.id)
            .subscribe(() => {
